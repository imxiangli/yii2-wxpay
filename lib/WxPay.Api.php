...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetSign();
		$xml = $inputObj->ToXml();
        Yii::getLogger()->log('unifiedOrder xml:'.$xml, \yii\log\Logger::LEVEL_INFO);
		
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, false, $timeOut);
        Yii::getLogger()->log('unifiedOrder rs:'.$response, \yii\log\Logger::LEVEL_INFO);
	...
	public static function orderQuery($inputObj, $timeOut = 6)
	{
		...
		$startTimeStamp = self::getMillisecond();//请求开始时间
		$response = self::postXmlCurl($xml, $url, false, $timeOut);
		Yii::getLogger()->log('orderQuery rs:'.$response, \yii\log\Logger::LEVEL_INFO);
		$result = WxPayResults::Init($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	...
	{		
		...
		if($data){
			...
			curl_close($ch);
			throw new WxPayException("curl出错，错误码:$error,".$error_msg);
		}
	}
